...
A key-down step's screenshot triple is cut from the newest retained frame on
its display inside the bounded window `(event_ts, event_ts + 250 ms]`, chosen
after a 100 ms settle (DEC-004; bounded wait on the capture worker, pinned
pre-event fallback per DEC-002), a click step's triple is unchanged, orderly
stop lets an accepted key-down finish its wait, and ADR-0001, the README,
and the `frame_age_ms` doc comments record the per-kind rule. This slice
supersedes PR-01 (PR #39, head `176be565676b5abc383f684a81597893c3260524`),
whose implementation of the earlier "oldest in-window frame" rule failed the
AC-001 gate on the first keystroke (GA-007); it starts by integrating that
commit and then applies the DEC-004 correction.

## Scope and Ownership

- Behavior: Implement DEC-001, DEC-002, and DEC-004 in the capture pipeline
  without changing its architecture. Start by fast-forwarding the slice
  branch onto the superseded PR-01 commit
  `176be565676b5abc383f684a81597893c3260524` (`origin/feat/pr-01-key-down-post-event-frames`,
  a direct descendant of the START base), whose broker range query, injectable
  wait runtime, explicit selected-frame packet assembly, stop reorder, and
  documentation are retained; then apply the DEC-004 correction on top:
  - `FrameBroker`'s pure bounded range query for one display returns the
    retained frame with the **largest** `ts_ns` such that
    `event_ts_ns < ts_ns <= deadline_ns` (deadline =
    `event_ts_ns + POST_EVENT_FRAME_WINDOW_NS`, 250 ms), or `None`. Choose
    the maximum eligible timestamp across the two retained slots (do not
    assume `previous` is older than `newest`). Equality with the event is
    not eligible; equality with the deadline is eligible. Use
  ...
  - `docs/adr/0001-pre-buffered-screen-capture.md`: revise in place the
    ...
    paint), the worker-side bounded wait, the pinned
...
- Primary execution flow: Tap callback pins the pre-event snapshot and
  ...
  key-down, bounded settle/wait then the newest in-window frame -> `build_packet`
...
- Rust (`cd src-tauri && cargo test`), all deterministic (fake clock and
  ...
  - broker: two in-window frames -> the larger timestamp; frame equal to
  ...
  - worker: an early in-window frame (for example event + 30 ms) followed
    ...
    worker after its deadline with an in-window frame retained -> that
    frame with zero waits; job after its deadline with only an
    after-deadline frame -> pinned immediately;
...
- Selection predicates: asynchronous, stateful work (worker settle/wait
  against a concurrently advancing broker; timing, ordering, and shutdown
  edge cases) across multiple files
