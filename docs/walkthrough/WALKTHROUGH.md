...
- A ListenOnly `CGEventTap` on its own run loop thread sees the input. It
  ...
  pins the newest buffered frame and hands the event to a bounded first-in, first-out (FIFO)
  queue.
- One ScreenCaptureKit stream per display runs continuously at about 10
  frames per second (fps) and keeps the two newest frames in memory. That is what makes a
  click screenshot show the menu *before* the click closed it. On-demand
  capture after the event was rejected for that reason (architecture decision
  record [ADR-0001](../adr/0001-pre-buffered-screen-capture.md)).
...
The `Hello World!` typing in the video is the hard case. For a key press the
...
real-recording gates to get right; see [How it was built](#how-it-was-built).
...
modifier state only, never timing ([ADR-0002](../adr/0002-key-event-semantic-classifier.md)). Its verdicts are not
...
Back on the list, the workflow you saved is at the top with its thumbnail, name,
...
confirmed hard delete ([ADR-0003](../adr/0003-hard-delete-for-saved-workflows.md)); there is no trash lifecycle in the
...
2. Each capability then ran interview → ChatGPT Pro plan → Codex consensus
   ...
   per pull request (PR) → independent exact-head review → merge. Eight PRs,
   seven merged.
...
(issue #43). The unit tests passed all three times. Two of the rules only
...
two stretch items not built. Both would sit on top of the classifier that
already ships. Automatic `wait`/`assert` detection, re-parse of the
