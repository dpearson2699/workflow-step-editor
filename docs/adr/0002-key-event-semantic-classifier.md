# Key-event semantic classifier in the core
...
shortcuts) need exactly the same distinction as their boundary rule. The
project therefore ships one pure, stateless, unpersisted key-event semantic
classifier (working name `KeySemantics`) in the minimum viable product (MVP)
recording and parser core, and every consumer routes through it: auto-titles
first, burst boundaries and toggle filtering later. Chord detection uses
modifier state only (any held non-Shift modifier), never timing thresholds: users may hold modifiers
