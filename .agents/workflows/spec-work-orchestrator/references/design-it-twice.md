...
When an alternative-interface exploration is wanted — a decision-shaped gray
area about a module's interface or seam during Discuss, or a consequential
interface choice during Plan slice decomposition — use this parallel sub-agent
pattern. Based on "Design It Twice" (Ousterhout) — your first idea is unlikely
to be the best.

Uses the vocabulary in [codebase-design.md](./codebase-design.md) — **module**,
**interface**, **seam**, **adapter**, **leverage**.

The sub-agents are bounded read-only support descendants; they never ask the
user and never close a decision. The root synthesizes the comparison. When the
exploration serves a decision-shaped gray area, the comparison and
recommendation feed that question's options and recommended answer, and the
user decides.
...
Include both [codebase-design.md](./codebase-design.md) vocabulary and
CONTEXT.md vocabulary in the brief so each sub-agent names things consistently
with the architecture language and the project's domain language.
