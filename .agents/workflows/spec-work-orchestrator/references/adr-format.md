...
An owning repository's existing ADR convention — location, file naming, and
structure — governs when one exists. The rules below are the default for
...
Policy enumeration, evidence logs, and open questions belong to their canonical
owners (domain docs and invariants, audits, and the interview ledger), not to
the ADR; link them instead of inlining them.
