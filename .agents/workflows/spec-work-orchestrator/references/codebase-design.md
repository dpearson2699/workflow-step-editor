...
"boundary." Consistent language is the whole point. The rule governs the
architectural nouns you choose; it never renames real identifiers or domain
terms — an `APIClient` or `PaymentService` keeps its exact name, and a
`CONTEXT.md` term wins over any word here.
