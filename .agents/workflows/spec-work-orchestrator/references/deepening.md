...
Assumes the vocabulary in [codebase-design.md](./codebase-design.md) —
...
- Old unit tests on shallow modules become waste once tests at the deepened
  module's interface exist — delete them, but only after showing that each
  distinct invariant an old test protected (a prior root-cause regression
  especially) is asserted by a test at the new interface. An invariant with
  no new home keeps its old test until it has one.
